library(readxl)
library(data.table)

worksheets <- c("Males","Females")

for (w in worksheets) {
  data <- readxl::read_excel("data-raw/ukmidyearestimates20182019ladcodes.xls",
                             sheet = paste0("MYE2 - ",w),
                             range = "A5:CQ9")
  setDT(data)

  data <- data[Name == "ENGLAND",-c("Code","Name","Geography1","Geography","All ages")]
  data[, Sex := w]

  data <- melt(data, id.vars = "Sex", value.name = "population", variable.name = "Age")

  assign(paste0("data_",w), data)
}

pop_counts <- rbindlist(list(data_Males,data_Females))


pop_counts <- pop_counts[, Sex := factor(Sex,
                                         levels = c("Females","Males"),
                                         labels = c("Female","Male"))]

pop_counts[, Age := as.numeric(Age) - 1]

pop_counts[ , Ageband := c("0-15" ,"16-24", "25-34", "35-44", "45-54", "55-64", "65+")[findInterval(Age, c(0, 16, 25, 35, 45, 55, 65, 1000))]]

pop_counts_age_sex <- copy(pop_counts)

usethis::use_data(pop_counts_age_sex,overwrite = T)

